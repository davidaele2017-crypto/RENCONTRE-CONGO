...
- Sortie audio (écouteur/haut-parleur) : sur mobile, un appel vocal ressort par défaut sur le haut-parleur (comportement du navigateur, pas de l'app) au lieu de l'écouteur comme un vrai appel téléphonique. Dès que la connexion est établie, `calls.js` bascule automatiquement sur l'écouteur si l'appareil en expose un distinct, et affiche un bouton 🔈/🔊 pour basculer manuellement — via `HTMLMediaElement.setSinkId()`, standard mais au support inégal : ça marche sur Chrome desktop et la plupart des Android récents, **mais pas sur Safari iOS** (l'OS y gère seul la sortie audio, aucune API web ne permet de la piloter) — sur les appareils sans cette API ou sans sortie distincte détectée, le bouton n'apparaît simplement pas plutôt que de proposer un réglage qui ne ferait rien
- Plein écran pour les appels **vidéo** : un bouton ⛶ en haut à droite, ou un simple clic/tap direct sur la vidéo, bascule en plein écran natif du navigateur (masque la barre d'adresse, pas juste notre interface qui occupe déjà tout l'écran en CSS) — comme sur les apps de messagerie modernes. Via `Element.requestFullscreen()` (Chrome/Firefox/Edge, desktop et Android) avec repli sur `HTMLVideoElement.webkitEnterFullscreen()` sur Safari iOS (qui ne sait mettre en plein écran qu'une balise `<video>`, pas un élément quelconque — l'app détecte laquelle des deux API est disponible et s'adapte)
